(function() {
    ...
    angular.module('app.services.function', ['ngCookies'])
        ...
        .constant('globalConfig', {
            webChatDebug: false,
            apiUrl: 'http://devqyftapi.snsshop.net',//测试http://devqyftapi.snsshop.net 正式http://qyftapi.vikduo.com
            uploadUrl: 'http://devqyftapi.snsshop.net/',//测试http://devqyftapi.snsshop.net/ 正式http://qyftapi.vikduo.com/
            debug: false
        })
        ...
        .service('globalFunction', ['globalConfig', '$resource', function(globalConfig, $resource) {
            ...
            this.createResource = function(url, param_defaults, actions) {
                ...
                if (localStorage.token) {
                    inner_param_defaults.PHPSESSID = localStorage.token;
                }
            ...
            function(commonUserApi, $q, $cookies, globalFunction, globalConfig, $location, user) {
                ...
                this.restorageUserInfo = function(data) {
                    if (data)
                        ...
                        _.extend(user, JSON.parse(localStorage.user));
                    else if ($cookies.user) {
                        localStorage.setItem('user', $cookies.user);
        ...
        .factory('weChat', ['$http', '$q', '$timeout', 'weChatConfig', 'globalConfig', function($http, $q, $timeout, weChatConfig, globalConfig) {
            ...
            var is_ready = false;
            //获取JS-SDK使用权限签名
            $http.get(globalConfig.apiUrl + weChatConfig.apiUrl + "?app_code=approval&PHPSESSID=" + localStorage.token).then(function(response) {
