...
function get_next_number(join,num)
{
    var have_in = false;
...
DUOBAO.chooseQty = {
	...
	increase: function(ele){
        ...
        for(var i = this.now_num; i <= this.max; i++)
        {

            if(get_next_number(this.join,i))
            {
                this.now_num = i;
                break;
            }
		...
		if (this.max && number > this.max) {
			...
			alert('本奖品最多可由'+ this.max +'人参与,参与人数已达最大值哦', {title: false, closeBtn: 0}, function(index){
                $(ele).parent().find('input.quantity').val(this.max);
			});
            return;
		}

        if(this.total_coupon != 0)
        {
            $(".each-limit i").html(this.total_coupon/number);
            $(".total i").html(this.total_coupon/number);
            if(this.user_coupon != 0)
            {
                if((this.user_coupon - this.total_coupon/number) < 0)
                {
                    $("#suc_pay_button").hide();
                    $("#no_pay_button").show();
                }
	...
	decrease: function(ele){
        ...
        for(var i = this.now_num; i >= this.min; i--)
        {
            if(get_next_number(this.join,i))
            {
                this.now_num = i;
		...
		if (number < this.min) {
			...
			alert('本奖品最少需'+ this.min +'人参与', {title: false, closeBtn: 0}, function(index){
	...
	txtEntry: function(ele, empty){
		...
		if (! this.regInt.test(trim(number))) {
			...
			alert('格式错误', {title: false, closeBtn: 0}, function(index){
	...
	init: function(callback) {
        if(this.user_coupon - (this.total_coupon/this.now_num) < 0)
        {
            $("#suc_pay_button").hide();
            $("#no_pay_button").show();
        }
...
DUOBAO.popWin = {
	...
	rootHtml: function(prizeName, no, exchangeEntryUrl){
		...
		html.push('<img src="images/popWin/bg_win.jpg" width="217" height="187" alt="">');
...
DUOBAO.popWinShare = {
	...
	hide: function(){
		$('.pop-mask-share, .pop-share').remove();
	},
	close: function(){
		var e = this;
		$('.pop-close').on('click', function(){
