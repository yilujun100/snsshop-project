...
(function($){
	...
	btnNext.on('click', function(){
		if ($(this).hasClass('btn-form-next-disabled')) return false;
		// @后台: 提交相关数据
        var captcha = $('#verifyCode').val();
        var mobile = $('#phoneNum').val();
        var channel_source = $('#channel_source').val();
        DUOBAO._post(DUOBAO.url.register,{captcha:captcha,mobile:mobile,channel_source:channel_source},function(res){
            if (res.retCode == 0) {
		...
		if (isEmpty('#verifyCode')) {
            ...
            if (!regExp.code.test(trim(e.val()))) {
                ...
                if ($('#phoneNum').hasClass('phone-error')) {
                    ...
                    showInfoTips();
                    btnNext.removeClass('btn-form-next-disabled');
                }
			}
		}		
	}
