$(function () {
	...
	$('#actCustomTab > a').on('click', function() {
		...
		DUOBAO._get(url, function (rs) {
            ...
            if(rs.retData.list == '')
            {
                ...
                html.push('<img src="'+img_url+'images/empty_pic.png" width="82" height="82" alt="">');
            ...
            if(rs.retData.list == '')
            {
                $('#actCustomCon > div').eq(index).addClass('empty').show();
            }
            else
            {
	...
	function record_tmpl (record) {
		...
		if (record.sGoodsName.length > 10) {
            if(record.sActName.length > 10)
            {
                html.push('<div class="prize-name">' + record.sGoodsName.substring(0, 11) + '...['+ record.sActName.substring(0, 11) +']</div>');
            }
            else
            {
                html.push('<div class="prize-name">' + record.sGoodsName.substring(0, 11) + '...['+ record.sActName +']</div>');
            }

		} else {
            if(record.sActName.length > 10)
            {
                html.push('<div class="prize-name">' + record.sGoodsName + '['+ record.sActName.substring(0, 11) +']</div>');
            }
            else
            {
                html.push('<div class="prize-name">' + record.sGoodsName + '['+ record.sActName +']</div>');
            }
