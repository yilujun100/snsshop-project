...
class WxPayConf_pub
{
    ...
    //const APPID = 'wx8e4c7364a259befb';
    //受理商ID，身份标识
    //const MCHID = '1216432801';
    //商户支付密钥Key。审核通过后，在微信发送的邮件中查看
    //const KEY = '6abf4c7f95553d90247c32d09da5e76a';
    //JSAPI接口中获取openid，审核后在公众平台开启开发模式后可查看
    //const APPSECRET = '70eec0964d1cad3dcc698272003c78a1';
    ...
    //=======【证书路径设置】=====================================
    //证书路径,注意应该填写绝对路径
    //const SSLCERT_PATH = '/sslcert/weixin_cert.pem';
    //const SSLKEY_PATH = '/sslcert/weixin_key.pem';
    //=======【异步通知url设置】===================================
    //异步通知url，商户根据实际开发过程设定
    //const NOTIFY_URL = 'http://weituangou.vikduo.com/test/notify';
    ...
    static function getConfig($key){
        if(self::$config == null){
            ...
            self::$config = config_item('weixinPay');
        }
...
class Common_util_pub
{
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".WxPayConf_pub::getConfig('KEY');
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, WxPayConf_pub::getConfig('SSLCERT_PATH'));
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, WxPayConf_pub::getConfig('SSLKEY_PATH'));
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
		...
		if($data){
			...
			$error = curl_errno($ch);
            log_message('error', 'postXmlSSLCurl failed | error no '.$error.' | '.json_encode(curl_getinfo($ch)));
			echo "curl出错，错误码:$error"."<br>"; 
			echo "<a href='http://curl.haxx.se/libcurl/c/libcurl-errors.html'>错误原因查询</a></br>";
			curl_close($ch);
			return false;
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function createXml()
	{
	   	$this->parameters["appid"] = WxPayConf_pub::getConfig('APPID');//公众账号ID
	   	$this->parameters["mch_id"] = WxPayConf_pub::getConfig('MCHID');//商户号
        $sub_appid = WxPayConf_pub::getConfig('SUB_APPID');
        $sub_mch_id = WxPayConf_pub::getConfig('SUB_MCHID');
        if (!empty($sub_appid) && !empty($sub_mch_id)) {
            $this->parameters["sub_appid"] = WxPayConf_pub::getConfig('SUB_APPID');//子商户公众账号ID
            $this->parameters["sub_mch_id"] = WxPayConf_pub::getConfig('SUB_MCHID');//子商户号
        }
	...
	{
	    $xml = $this->createXml();
        //get_instance()->log->notice('test','$xml['.$xml.'] | '.__METHOD__);
...
{	
	function __construct() 
	{
		...
		//设置curl超时时间
		$this->curl_timeout = WxPayConf_pub::getConfig('CURL_TIMEOUT');
	}
	...
	function createXml()
	{
		try
		{
			...
			if($this->parameters["out_trade_no"] == null) 
			{
                ...
                (empty($this->parameters["openid"]) && empty($this->parameters["sub_openid"]))){
	...
	{
		$this->postXml();
		//WTG_Lib_Log::debug('test', '$this->response: '. $this->response);
		$this->result = $this->xmlToArray($this->response);
		$prepay_id = isset($this->result["prepay_id"]) ? $this->result["prepay_id"] : 0;
		return $prepay_id;
	}
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = WxPayConf_pub::getConfig('APPID');//公众账号ID
		   	$this->returnParameters["mch_id"] = WxPayConf_pub::getConfig('MCHID');//商户号
...
class JsApi_pub extends Common_util_pub
{
	...
	{
		$urlObj["appid"] = WxPayConf_pub::getConfig('APPID');
	...
	{
		$urlObj["appid"] = WxPayConf_pub::getConfig('APPID');
		$urlObj["secret"] = WxPayConf_pub::getConfig('APPSECRET');
	...
	{
		$jsApiObj["appId"] = WxPayConf_pub::getConfig('APPID');
		$timeStamp = time();
