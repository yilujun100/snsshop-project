    ...
    <table class="table mt-10 fr">
        ...
        <?php } else { foreach ($result_list['list'] as $v) { ?>
            <tr data-id="<?=$v['sOrderId']?>">
                ...
                <td><?=$v['iCount']?></td>
                <td><?=$v['iPresentCount']?></td>
                <td><?=$v['iUin']?></td>
                <td><?=$v['sNickName']?></td>
                <td><?=price_format($v['iTotalPrice'])?></td>
                <td><?=Lib_Constants::$pay_status[$v['iPayStatus']]?></td>
                <td><?php $this->widget('button', array('node'=>'coupon_order/index','button'=>Lib_Constants::BTN_DETAIL,'class'=>'order-detail'))?></td>
            </tr>
        <?php }}?>
        </tbody>
...
<script type="text/javascript">
    $(function () {
        ...
        $search_do.click(function () {
            ...
            $.yRedirect('<?=node_url('coupon_order/index')?>' + '?' + search_str);
        });
        $search_rest.click(function () {
            $.yRedirect('<?=node_url('coupon_order/index')?>');
        });
        $detail.click(function () {
            var order_id = $(this).closest('tr').attr('data-id');
            $.yRedirect('<?=node_url('coupon_order/detail/')?>' +  order_id);
        });
    });
</script>