    ...
    <table class="table mt-10 fr">
        ...
        <?php } else { foreach ($result_list['list'] as $v) { ?>
            <tr data-id="<?=$v['sOrderId']?>">
                ...
                <td><?=Lib_Constants::$pay_status[$v['iPayStatus']]?></td>
                <td><?php $this->widget('button', array('node'=>'active_order/index','button'=>Lib_Constants::BTN_DETAIL,'class'=>'order-detail'))?></td>
            </tr>
        <?php }}?>
        </tbody>
        <tfoot><tr><td colspan="11"><?php $this->widget('pagination', $result_list)?></td></tr></tfoot>
    </table>
</div>
...
<script type="text/javascript">
    $(function () {
        ...
        $search_do.click(function () {
            ...
            $.yRedirect('<?=node_url('active_order/index')?>' + '?' + search_str);
        });
        $search_rest.click(function () {
            $.yRedirect('<?=node_url('active_order/index')?>');
        });
        $detail.click(function () {
            var order_id = $(this).closest('tr').attr('data-id');
            $.yRedirect('<?=node_url('active_order/detail/')?>' +  order_id);
        });
    });
</script>