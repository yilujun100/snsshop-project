...
<div class="grid filter-wrap">
    <form action="" class="form-filter clearfix" id="search-form">
        <div class="form-item">
            ...
            <!--<p>
                <label for="variable_search_type">配置类型</label>
            ...
            <p>
                <label></label>
                <?php $this->widget('button', array('node'=>'config_activity/index','button'=>Lib_Constants::BTN_OK,'id'=>'variable-search-do'))?>
                <?php $this->widget('button', array('node'=>'config_activity/index','button'=>Lib_Constants::BTN_RESET,'id'=>'variable-search-reset'))?>
            </p>
        </div>
    </form>
</div>
...
<div class="table-con">
    <!--<span class="table-button fr">
        <?php /*$this->widget('button', array('node'=>'config_activity/add','button'=>Lib_Constants::BTN_ADD,'id'=>'variable-add'))*/?>
    </span>-->
    <table class="table mt-10 fr">
        <thead>
        <tr>
            <th width="15%">配置 Key</th>
            <th width="15%">配置名称</th>
            <th width="45%">配置值</th>
            <th width="10%">配置类型</th>
            <th width="20%">操作</th>
        ...
        <?php } else { foreach ($result_list['list'] as $v) { ?>
            <tr data-unique="<?=$v['sKey']?>">
                ...
                <td>
                    <?php $this->widget('button', array('node'=>'config_activity/edit','button'=>Lib_Constants::BTN_EDIT,'class'=>'variable-edit'))?>
                </td>
            </tr>
        <?php }}?>
        </tbody>
...
<div class="form-con" id="variable-popup" style="display: none">
    <form class="form" id="variable-form">
        <div class="f-item">
            <label for="variable_type">配置类型</label>
        ...
        <div class="f-item">
            <label for="variable_key">配置 Key</label>
            <input type="text" name="variable_key" id="variable_key" placeholder="配置 Key">
        </div>
        <div class="f-item">
            <label for="variable_name">配置名称</label>
            <input type="text" name="variable_name" id="variable_name" placeholder="配置名称">
        </div>
        <div class="f-item">
            <label for="variable_value">配置值</label>
            <textarea name="variable_value" id="variable_value" placeholder="配置值" style="width:520px; height: 200px; overflow-y: auto;"></textarea>
        </div>
        <div class="f-item">
...
<script type="text/javascript">
    $(function () {
        var $search_do = $('#variable-search-do'),
            ...
            $popup = $('#variable-popup').yForm('新增/编辑配置', 960),
            $form = $('#variable-form'),
            $add = $('#variable-add'),
            $edit = $('.variable-edit'),
        ...
        $edit.click(function () {
            ...
            $this.yAjax({
                url: '<?=node_url('config_activity/get_item')?>',
                data: {unique: unique},
        ...
        $delete.click(function () {
            ...
            $.yConfirm('确认要删除该配置吗?', function () {
                $this.yAjax({
                    url: '<?=node_url('config_activity/delete')?>',
                    data: {unique: unique},
                    success: function (data) {
                        if (0 === data.retCode) {
                            $.ySuccess('删除成功');
                            $.yRedirect('<?=node_url('config_activity/index')?>', 1000);
                        } else {
                            $.yError(data.retMsg);
                        }
                    }
        ...
        validator = $form.validate({
            ...
            submitHandler: function () {
                ...
                if ('add' === op) {
                    url = '<?=node_url('config_activity/add')?>';
                } else {
                    url = '<?=node_url('config_activity/edit')?>';
                }
                $form.yAjax({
                    url: url,
                    data: $form.serialize(),
        ...
        $search_do.click(function () {
            ...
            $.yRedirect('<?=node_url('config_activity/index')?>' + '?' + search_str);
        });
        $search_rest.click(function () {
            $.yRedirect('<?=node_url('config_activity/index')?>');
        });
    });
</script>