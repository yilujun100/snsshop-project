...
<div class="grid filter-wrap">
    <form action="" class="form-filter clearfix" id="search-form">
        <div class="form-item">
            ...
            <p class="f-item">
                <label></label>
                <?php $this->widget('button', array('node'=>'robot/stat','button'=>Lib_Constants::BTN_OK,'id'=>'robot-search-do'))?>
                <?php $this->widget('button', array('node'=>'robot/stat','button'=>Lib_Constants::BTN_RESET,'id'=>'robot-search-reset'))?>
            </p>
        </div>
    </form>
</div>
    ...
    <table class="table mt-10 fr">
        <thead>
            <tr class="stat-title">
                ...
                <th style="border-left: 0"><?php $this->widget('button', array('node'=>'robot/excel','button'=>Lib_Constants::BTN_EXPORT_EXCEL,'id'=>'robot-excel'))?></th>
            </tr>
            <tr>
                <th width="9%">日期</th>
                <th width="9%">总开奖次数</th>
...
<script type="text/javascript">
    $(function () {
        ...
        $search_do.click(function () {
            ...
            $.yRedirect('<?=node_url('robot/stat')?>' + '?' + search_str);
        });

        $search_rest.click(function () {
            $.yRedirect('<?=node_url('robot/stat')?>');
        });
        ...
        $excel.click(function () {
            $.yConfirm('确定要下载该数据文件吗?', '确认', function () {
                ...
                search_str = $.param(search);
                $.yRedirect('<?=node_url('robot/excel')?>' + '?' + search_str);
            });
        });
    });
</script>