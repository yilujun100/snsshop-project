        ...
        <tr data-id="<?=$v['iAutoId']?>">
            ...
            <td>
                <?php if (Lib_Constants::DELIVER_NOT_CONFIRM_STATUS == $v['iDeliverStatus']) {
                    $this->widget('button', array('node'=>'deliver/groupon_ok','button'=>Lib_Constants::BTN_DELIVER_CONFIRM,'class'=>'deliver_confirm'));
                }
                $this->widget('button', array('node'=>'deliver/groupon_detail','button'=>Lib_Constants::BTN_DETAIL,'class'=>'deliver_detail'));
                ?>
            </td>
        </tr>
        <?php }}?>
...
<script type="text/javascript">
    $(function () {
        ...
        $search_do.click(function () {
            ...
            $.yRedirect('<?=node_url('deliver/groupon')?>' + '?' + search_str);
        });
        $search_rest.click(function () {
            $.yRedirect('<?=node_url('deliver/groupon')?>');
        });

        $detail.click(function () {
            var deliver_id = $(this).closest('tr').attr('data-id');
            $.yRedirect('<?=node_url('deliver/groupon_detail/')?>' +  deliver_id);
        });
        ...
        $form.validate({
            ...
            submitHandler: function () {
                ...
                $form.yAjax({
                    url: '<?=node_url('deliver/groupon_ok')?>',
                    data: $form.serialize(),
