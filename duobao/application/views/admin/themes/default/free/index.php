...
<div class="table-con">
    ...
    <table class="table mt-10 fr">
        ...
        <?php } else { foreach ($result_list['list'] as $v) { ?>
            <tr>
                <td><?=period_code_encode($v['iActId'],$v['iPeroid'])?></td>
                <td><?=$v['iGoodsId']?></td>
                ...
                <td><?=Lib_Constants::$active_lot[$v['iLotState']]?></td>
                <td>
                    <?php if($v['iLotState'] == 0){?>
                    <a href="javascript:;" data-node="free/lottery" class="btn-opera btn-opera-info" onclick="ADMIN.Opts.lottery('<?=period_code_encode($v['iActId'],$v['iPeroid'])?>','free', 1)">强制开奖</a>
                    <?php } ?>
                </td>
            </tr>
        <?php }}?>
...
<script>
    ...
    ADMIN.Opts.lottery = function($id, $class, $opt, $cb) {
        layer.confirm('确认要强制开奖吗?', {
            title: false,
            closeBtn: false,
            btn: ['确认', '取消']
        }, function () {
            $uri = '/admin/'+$class+'/lottery';
