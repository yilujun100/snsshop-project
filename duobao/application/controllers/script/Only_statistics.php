...
class Only_statistics extends Script_Base
{
    const LIMIT = 1000;  //每次脚本处理的数量
    const REPEAT = 3;//操作失败，则重复操作次数

    protected $log_type = 'Only_statistics';
    ...
    public function run()
    {
        ...
        for($i=1;$i<=180;$i++)
        {
            $toDayTime =  $time - 60*60*24*$i;   //当天凌晨时间
            $yesterDayTime =  $time - 60*60*24*($i+1);   //前一天凌晨时间
            ...
            for($user_index = 0; $user_index < 10; $user_index ++)
            {
                ...
                $user_sql = 'SELECT count(iUin) cTotal FROM '.$user_db_name.'  ;';
