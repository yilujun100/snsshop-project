 ...
 * Class Active_order
 */
class Active_order extends Admin_Base
{
	...
	public function index()
	{
		...
		if (empty($where)) {
			...
			$result_list = $this->active_order_model->row_list('*', $where, $order_by, $page);
			if ($result_list['count'] > 0) {
				$this->load->model('goods_item_model');
				$this->load->model('user_model');
				foreach ($result_list['list'] as & $v) {
					$goods = $this->goods_item_model->get_row($v['iGoodsId']);
					$v['sGoodsName'] = $goods['sName'];
	...
	{
		$this->load->model('active_order_model');
		$item = $this->active_order_model->get_row($order_id);
		...
		$goods = $this->goods_item_model->get_row($item['iGoodsId']);
		$item['sGoodsName'] = $goods['sName'];
