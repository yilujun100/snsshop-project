...
class Active_order extends MY_Controller
{
    /**
     * 构造函数
     *
        ...
        for($i=0;$i<10;$i++){
            ...
            for($j=0;$j<10;$j++){
                ...
                while($row = mysql_fetch_array($query)){
                    ...
                    $user = $this->user_model->get_row(array('sOpenId'=>$user['openid']));
                    $table = $this->user_model->map($user['iUin'])->get_cur_table();
                    $table_ext = substr($table,-1);
                    $merageOrderId = $this->setOrderId(Lib_Constants::PLATFORM_WTG,$row['iType'],$user['iUin']);
                    $merage['t_active_merage_order'.$table_ext][] = array(
                        ...
                        'iCoupon' => $row['iCashCoupon'],
                        'iAmount' => Lib_Constants::COUPON_UNIT_PRICE*($row['iCount']-$row['iCashCoupon']),
                    ...
                    //查询订单
                    $active_config = $this->active_config_model->get_row(array('iGrouponId'=>$row['iGrouponId']));
                    if(empty($active_config)){
                        echo '================not found active config=============</br>';
                        pr($row);continue;
                    }
                    $summary = $this->luckycode_summary_model->get_row(array('sOrderId'=>$row['sOrderId'],'iActId'=>$active_config['iActId']));
                    if(empty($summary)){
